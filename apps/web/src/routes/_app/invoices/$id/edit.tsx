...
export const Route = createFileRoute('/_app/invoices/$id/edit')({
  head: () => ({
    meta: [{ title: 'Edit Invoice | Invoice' }, { name: 'robots', content: 'noindex, nofollow' }],
  }),
...
function EditInvoiceContent() {
  ...
  useEffect(() => {
    if (invoice.data && !isInitialized) {
      ...
      setDiscountValue(
        inv.discountType === 'fixed' ? (inv.discountValue || 0) / 100 : inv.discountValue || 0
      );
      setNotes(inv.notes || '');
  ...
  const updateLineItem = (lid: string, field: keyof LineItem, value: string | number) => {
    setLineItems(lineItems.map((item) => (item.id === lid ? { ...item, [field]: value } : item)));
  };
  const removeLineItem = (lid: string) => {
    if (lineItems.length === 1) {
      toast.error('Invoice must have at least one line item');
      return;
    }
    setLineItems(lineItems.filter((item) => item.id !== lid));
  };
  ...
  if (invoice.data.status !== 'draft') {
    return (
      <div className="min-h-full">
        <div className="container mx-auto max-w-4xl px-6 py-10">
          <div className="text-center py-20">
            <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center bg-amber-500/10">
              <FileText className="h-8 w-8 text-amber-500" />
            </div>
            <h2 className="font-display text-2xl mb-2">Cannot Edit Invoice</h2>
            <p className="text-sm text-muted-foreground mb-6">Only draft invoices can be edited.</p>
            <Button
              ...
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={invoiceNumber}
                    disabled
                    className="bg-muted/30 border-border/40 h-10 font-mono text-sm"
                  />
            ...
            <CardHeader className="border-b border-border/40 bg-muted/20 py-3.5 px-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className="text-xs">
                    Items or services you're billing for
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
